...
def render(scene, pixels)
  ...
  HEIGHT.times do |y|
    ...
    WIDTH.times do |x|
      ...
      r = Math.min(255, (pixel.x * 255.0).round.to_i).to_u32
      g = Math.min(255, (pixel.y * 255.0).round.to_i).to_u32
      b = Math.min(255, (pixel.z * 255.0).round.to_i).to_u32
      pixels[i] = (b << 24) + (g << 16) + (r << 8)
      i += 1
    end
  end
end
...
window = SDL::Window.new("Raytracer", WIDTH, HEIGHT, flags: SDL::Window::Flags.flags(SHOWN, RESIZABLE))
